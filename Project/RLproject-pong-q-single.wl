...
(*---------------------*)
game[ep_Integer,st_Integer,net_,render_, rand_, $env_]:= Module[{
		states, list,next,observation, punish,
	...
	Do[
		...
		Do[
			...
			If[ob=={},observation = Join[observation,state["Observation"]],
			observation = Join[observation,ob[[-1]][[;;4]]]];
			
		    If[RandomInteger[{0,100}]<=Max[rand*100,10],
		    action = RandomInteger[{0,1}];,
			...
			observation = Join[observation,state["Observation"]];
			observation = Join[observation,ob[[-1]][[;;4]]];
			AppendTo[next,observation];]\:ff1b
...
generator := Function[(
	...
	len = Length[processed["action"]]-000;
	If[len > 0, 
	...
	temp2 = 0.99*Max[Values[#]]&/@#Net[processed["next"][[pos]],"Probabilities"];
	temp = temp1 + temp2;
	(*hardcoded for pong*)
	MapIndexed[
